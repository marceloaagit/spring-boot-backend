...
public class ClienteInsertValidator implements ConstraintValidator<ClienteInsert, ClienteNewDTO> {
	...
	public boolean isValid(ClienteNewDTO objDto, ConstraintValidatorContext context) {
		...
		if(objDto.getTipo().equals(TipoCliente.PESSOAFISICA.getCod()) && !CpfOuCnjpValidUtil.isValidCpf(objDto.getCpfOuCnpj())) {
			list.add(new FieldMessage("cpfOuCnpj", "CPF inválido!!!"));
		}
		
		if(objDto.getTipo().equals(TipoCliente.PESSOAJURIDICA.getCod()) && !CpfOuCnjpValidUtil.isValidCnpj(objDto.getCpfOuCnpj())) {
